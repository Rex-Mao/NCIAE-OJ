...
JNIEXPORT jobject JNICALL Java_cn_edu_nciae_judgecenter_manager_Runner_getRuntimeResult(
    ...
    std::cout << "[INFO] Time Limit: " << timeLimit << " MS " << std::endl;
    std::cout << "[INFO] Memory Limit: " << memoryLimit <<" KB "<< std::endl;
...
void setupIoRedirection(
    ...
    if ( outputFilePath != "" ) {
        int outputFileDescriptor = open(outputFilePath.c_str(), O_CREAT | O_WRONLY, 0777);
...
//        std::cout << "[DEBUG] Current Memory of PID# " << pid << ": " << currentUsedMemory << " KB" << std::endl;
        ...
        if ( memoryLimit != 0 && maxUsedMemory > memoryLimit ) {
            std::cout << "[DEBUG] Current Memory of PID# " << pid << " is out of the limit." << std::endl;
            killProcess(pid);
        }
        usleep(500);
